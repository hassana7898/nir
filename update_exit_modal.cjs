...
// 3. Update the print modal opening logic.
const modalOld = `                            <div>
...
const modalNew = `                            {calculatePrintPages(filteredExits).map((_, index) => (
...
code = code.replace(/handlePrint\('exit', filteredExits, \{ printDate: currentDate, generalDescription: printDescription, customPrintTitle, showPageTotals, showGrandTotal \}\);/,
